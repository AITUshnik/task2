...
void threadFunc(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int start, int end, int numOfSamples) {
...
int main(int argc, char **argv) {
    ...
    double blockSize = viewPlaneResolutionX / numberOfThreads;
    auto ts = hrc::now();
	...
	int end = blockSize * (i+1);
        thread thr(threadFunc, ref(scene), ref(viewPlane), ref(image), 
            viewPlaneResolutionX, viewPlaneResolutionY, start, end, numOfSamples);
